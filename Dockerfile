...
async function saveArtistSuccess(artist, listeners) {
  ...
  const isUnverifiedCandidate =
    artist.discovery_status === "candidate" && !artist.last_collected_at;

  // 新規候補が基準未満なら登録対象にせず削除する。
  // 既に履歴のある既存アーティストは削除せず、従来どおり再確認する。
  if (!isActive && isUnverifiedCandidate) {
    const { error } = await supabase
      .from("artists")
      .delete()
      .eq("id", artist.id);
    ensure(null, error, "discard below-threshold candidate");
    return { kept: false, reason: "below_threshold" };
  }
  ...
  return { kept: true, status: isActive ? "active" : "below_threshold" };
}
