...
async function heartbeatRun(runId, runToken) {
  const { data, error } = await retryTransientOperation(() => supabase.rpc("heartbeat_worker_run", {
    p_run_id: runId,
...
async function reconcileStaleRuns() {
  const { data, error } = await retryTransientOperation(() => supabase.rpc("reconcile_stale_worker_runs", {
