...
async function reserveRunQuota() {
  const { data, error } = await retryTransientOperation(() => supabase.rpc("reserve_run_quota", {
...
async function finalizeRunQuota(quota, usage) {
  const { error } = await retryTransientOperation(() => supabase.rpc("finalize_run_quota", {
