...
- 収集配分: 各回の先頭で未成功の`error`/`candidate`を最大150件、`below_threshold`を最大50件再確認し、残りを`active`へ使用
- ワーカー内の処理順: 新規探索2検索語 → プレイリスト巡回6件 → 月間リスナー収集。325分の実行枠で公開対象の5〜6日更新と候補キュー解消を両立
